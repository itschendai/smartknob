...
void Paint_DrawImage(const unsigned char *image, UWORD xStart, UWORD yStart, UWORD W_Image, UWORD H_Image) 
{
    ...
    for (j = 0; j < H_Image; j++) {
        for (i = 0; i < W_Image; i++) {
            if (xStart + i < Paint.WidthMemory && yStart + j < Paint.HeightMemory) { // Check bounds
                unsigned char pixel = *(image + j * W_Image + i);

                // Extracting the RGB components
                unsigned char r = (pixel & 0xE0) >> 5;  // Extract red (3 bits)
                unsigned char g = (pixel & 0x1C) >> 2;  // Extract green (3 bits)
                unsigned char b = (pixel & 0x03);       // Extract blue (2 bits)

                // Scale components more conservatively
                r = (r << 5) | (r << 2);  // Scale red (3 bits to 8 bits)
                g = (g << 5) | (g << 2) | (g >> 1);  // Scale green (3 bits to 8 bits)
                b = (b << 6) | (b << 4) | (b << 2) | b;  // Scale blue (2 bits to 8 bits)

                // Combine into a single 16-bit color value (assuming RGB565 format)
                uint16_t color = (r << 11) | (g << 5) | (b);

                // Set the pixel at the calculated position with the correct color
